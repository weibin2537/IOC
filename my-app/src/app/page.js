  ...
  const getTokensToBeClaimed = async () => {
    try {
      ...
      if (balance === zero) {
        ...
        setTokensToBeClaimed(BigInt(amount));
      }
    } catch (err) {
      console.error(err);
      setTokensToBeClaimed(zero);
          ...
          {walletConnected ? (
            <div>
              <div className={styles.description}>
                ...
                You have minted {ethers.formatEther(balanceOfCryptoDevTokens)} Crypto
                Dev Tokens
              </div>
              <div className={styles.description}>
                {/* Format Ether helps us in converting a BigNumber to string */}
                Overall {ethers.formatEther(tokensMinted)}/10000 have been minted!!!
              </div>
              {renderButton()}
            </div>
          ) : (
